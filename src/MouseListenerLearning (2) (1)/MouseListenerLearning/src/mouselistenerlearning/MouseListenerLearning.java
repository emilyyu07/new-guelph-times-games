...
public class MouseListenerLearning {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                System.out.print(board[i][j].getWord());
            }
            System.out.println("");
        }
        new MyFrame(board);
    ...
    public static boolean under(CrosswordTile[][] b, int i, int j){
        if (i < b.length-2) {
            if (!b[i][j].getWord().equals("/") && !b[i+1][j].getWord().equals("/") && !b[i+2][j].getWord().equals("/")) {
                if (i == 0) {
                    return true;
                } else {
                    return b[i-1][j].getWord().equals("/");
                }
            } else{
                return false;
            }
    ...
    public static boolean right(CrosswordTile[][] b, int i, int j){
        if (j < b[0].length-2) {
            if (!b[i][j].getWord().equals("/") && !b[i][j+1].getWord().equals("/") && !b[i][j+2].getWord().equals("/")) {
                if (j == 0) {
                    return true;
                } else {
                    return b[i][j-1].getWord().equals("/");
                }
            } else{
                return false;
            }
