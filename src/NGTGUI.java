...
January 14, 2025
The New Guelph Times games GUI
 */
...
public class NGTGUI extends javax.swing.JFrame {
    ...
    public NGTGUI() {
        initComponents();
        //create a new stats object to hold current user's progress
    ...
    //CODE BELOW PERTAINS TO CROSSWORD GAME--------------------------------------

    /**
     * Write the best times to an output file
     *
     * @param times - the string that is to be written into the file
     */
    public void writeTimes(String times) {
        // in case file is not able to be created
        try {
            // instantiate a new buffered write to write to the a file created in this project's folder
            BufferedWriter writer = new BufferedWriter(new FileWriter("BestCrosswordTimes.txt"));
            // write the times onto the file
            writer.write(times);
            // close the writer so the text is displayed
            writer.close();
        } catch (IOException e) {
            // if the file is unable to be created print an error
    ...
    public static boolean under(CrosswordTile[][] b, int i, int j) {
        ...
        if (i < b.length - 1) {
            ...
            if (!b[i][j].getLetter().equals("/") && !b[i + 1][j].getLetter().equals("/")) {
    ...
    public static boolean right(CrosswordTile[][] b, int i, int j) {
        ...
        if (j < b[0].length - 1) {
            // make sure there isn't an omitted square 2 spaces more into the word
            if (!b[i][j].getLetter().equals("/") && !b[i][j + 1].getLetter().equals("/")) {
    ...
    //CODE ABOVE PERTAINS TO CROSSWORD GAME-----------------------------------------
    ...
    private void btnRulesWordleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRulesWordleActionPerformed
        //create new window and drawing panel for wordle rules
    ...
    private void btnPlayWordleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPlayWordleActionPerformed
        //when the play button is clicked, create a new wordle game (pass a new statistic collector into it)
